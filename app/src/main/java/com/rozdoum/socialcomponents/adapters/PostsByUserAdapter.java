...
public class PostsByUserAdapter extends BasePostsAdapter {
    public static final String Attach = PostsByUserAdapter.class.getSimpleName();
    ...
    public void loadPosts() {
        if (!activity.has_connection()) {
            activity.display_snackBar(R.string.internet_connection_failed);
        ...
        OnDataChangedListener<Post> onPostsDataChangedListener = new OnDataChangedListener<Post>() {
            @Override
            public void ChangedIn_List(List<Post> list) {
                setList(list);
                callBack.onPostsListChanged(list.size());
            }
        };

        PostManager.getInstance(activity).receive_postssListByUser(onPostsDataChangedListener, userId);
    }

    public void removeSelectedPost() {
        Posts.remove(Posts_positionSelection);
        callBack.onPostsListChanged(Posts.size());
